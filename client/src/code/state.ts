...
export function createState(inParentComponent) {
  ...
  return {
    ...
    showMessage : async function(inMessage: IMAP.IMessage): Promise<void> {
      ...
      // format this.state.currentMailbox
      let mailbox: string = this.state.currentMailbox;
      mailbox = ['inbox', 'notes', 'boxer'].includes(mailbox.toLowerCase()) ?
                mailbox :
                mailbox.replace('[Gmail]/', '').replace('Boxer/', '').replace(' ', '');

      const mb: String = await imapWorker.getMessageBody(inMessage.id, mailbox);
    ...
    getMessages : async function(inPath: string): Promise<void> {
      ...
      this.state.showHidePleaseWait(true);
      const imapWorker: IMAP.Worker = new IMAP.Worker();
      const messages: IMAP.IMessage[] = await imapWorker.listMessages(['inbox', 'notes', 'boxer'].includes(inPath.toLowerCase()) ? 
                                                                      inPath : 
                                                                      inPath.replace('[Gmail]/', '').replace('Boxer/', '').replace(' ', ''));
      this.state.showHidePleaseWait(false);

      this.state.clearMessages();

      console.log('type of messages is ', typeof messages);
    ...
    deleteMessage : async function(): Promise<void> {
      ...
      this.state.showHidePleaseWait(true);
      const imapWorker: IMAP.Worker = new IMAP.Worker();
      await imapWorker.deleteMessage(this.state.messageID, mailbox);
      this.state.showHidePleaseWait(false);
